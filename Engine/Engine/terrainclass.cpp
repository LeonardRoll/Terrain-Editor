...
TerrainClass::TerrainClass()
{
	...
	m_Texture = 0;
	m_Topology = false;
	m_RaiseLevel = 0; //No Raise
	m_SmoothLevel = 1;  //0% Smooth
	m_Range = 100;
}
...
void TerrainClass::IncreaseSmooth(ID3D11Device* device,bool apply){
	if(apply)
	{
		m_SmoothLevel -= 0.1; //10% increments
		GlobalTerrainSmooth(device);
	}
}
...
void TerrainClass::PerformRaise(bool apply,float x,float z,ID3D11Device* device)
{
	if(apply){
		LocalZoneRaise(x,z,device);
	}	
}

bool TerrainClass::LocalZoneRaise(float x,float z,ID3D11Device* device)
{
	int index;
	for(int i=0; i<m_terrainHeight; i++)
	{
		for(int j=0; j<m_terrainWidth; j++)
		{
			...
			if(((x + m_Range) >= j) && ((x - m_Range) <= j)){
				if(((z + m_Range) >= i) && ((z - m_Range) <= i))
	...
	CalculateNormals();
	CalculateTextureCoordinates();
	return GlobalTerrainSmooth(device);
}
...
void TerrainClass::SwitchTopology(bool input)
{
	m_Topology = input;
}
...
bool TerrainClass::GlobalTerrainSmooth(ID3D11Device* device)
{
	if(m_SmoothLevel < 0)
		m_SmoothLevel = 0;
	unsigned long* indices;
	...
	for(j=0; j<(m_terrainHeight-1); j++)
	{
		for(i=0; i<(m_terrainWidth-1); i++)
		{
			...
			m_vertices[index].position = D3DXVECTOR3(m_heightMap[index3].x, m_heightMap[index3].y * m_SmoothLevel , m_heightMap[index3].z);
			...
			m_vertices[index].position = D3DXVECTOR3(m_heightMap[index4].x, m_heightMap[index4].y * m_SmoothLevel , m_heightMap[index4].z);
			...
			// Bottom left.
			m_vertices[index].position = D3DXVECTOR3(m_heightMap[index1].x, m_heightMap[index1].y * m_SmoothLevel , m_heightMap[index1].z);
			...
			m_vertices[index].position = D3DXVECTOR3(m_heightMap[index2].x, m_heightMap[index2].y * m_SmoothLevel , m_heightMap[index2].z);
